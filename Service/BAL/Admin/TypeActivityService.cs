...
namespace Service
{
    public class TypeActivityService : BaseAdminService<TypeActivityModelView, TypeActivity>
    {
        ...
        {
            return repo.Db.Get(e => e.Name.ToLower().Trim().Equals("" + textSearch.ToLower().Trim()), Includes).Map<TypeActivityModelView>();          
        }

        public bool CheckDoublicat(string TypeActivityName, long id)
        {
            var ob = repo.Db.Get(e => e.Name.Equals("" + TypeActivityName) && (id == 0 || e.Id != id)).Map<TypeActivityModelView>();
