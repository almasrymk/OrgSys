...
import { DealerGroup, DealerType } from '../../models/dealer-group.model';
import { DealerGroupService } from '../../services/dealer-group.service';

const PAGE_SIZE = 20;

/** Replaces Areas/Setting/Views/DealerGroup/{Index,List}.cshtml. One component drives both "Client Groups" and "Supplier Groups". */
@Component({
  selector: 'app-dealer-group-list',
  standalone: true,
...
export class DealerGroupListComponent {
  ...
  readonly groups = signal<DealerGroup[]>([]);
  ...
  load(): void {
    ...
    this.service.search({ keySearch: this.keySearch(), typeId: this.dealerType, page: this.page(), pageSize: PAGE_SIZE }).subscribe({
      next: (result) => {
        ...
        this.groups.set(result.response ?? []);
  ...
  async onDelete(group: DealerGroup): Promise<void> {
    const confirmed = await this.confirmDialog.confirm(`Delete "${group.name}"? This cannot be undone.`, `Delete ${this.title.slice(0, -1)}`);
    if (!confirmed) return;
