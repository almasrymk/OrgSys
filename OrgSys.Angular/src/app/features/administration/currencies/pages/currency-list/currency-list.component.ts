...
/** Replaces Areas/Setting/Views/Currency/{Index,List}.cshtml. */
@Component({
...
export class CurrencyListComponent {
  ...
  load(): void {
    ...
    this.currencyService.search({ keySearch: this.keySearch(), page: this.page(), pageSize: PAGE_SIZE }).subscribe({
      next: (result) => {
        this.loading.set(false);
        this.currencies.set(result.response ?? []);
        this.pageCount.set(result.pageCount || 1);
      },
      error: () => {
        this.loading.set(false);
        this.toast.error('Could not load currencies.');
      },
    });
  }
  ...
  async onDelete(currency: Currency): Promise<void> {
