namespace OrgSys.Areas.Setting.Controllers
{
    ...
    [Area("Setting")]
    public class DistrictController(IConfiguration configuration, IMapper mapper) : MainController<DistrictDto, CreateDistrictCommand, UpdateDistrictCommand>(configuration, mapper)
    {
        public override async Task LoadViewBag(DistrictDto model)
        {
        ...
        {
            var districtes = await GetListApi<DistrictDto>($"GetListByCityId?CityId={CityId}&Page=1&PageSize=20");
        ...
        public async Task<JsonResult> GetList(string txtSearch = "", int page = 1, int pageSize = 20)
        {
            ...
            var itemsList = await GetListApi<DistrictDto>(TextSearch: txtSearch, Page: page, PageSize: pageSize);
