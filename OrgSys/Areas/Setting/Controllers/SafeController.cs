namespace OrgSys.Areas.Setting.Controllers
{
    ...
    [Area("Setting")]
    public class SafeController(IConfiguration configuration, IMapper mapper) : MainController<SafeDto, CreateSafeCommand, UpdateSafeCommand>(configuration, mapper)
    {
        public override async Task LoadViewBag(SafeDto model)
        {
        ...
        public override async Task<SafeDto> InitializeData(SafeDto ob)
        {
            ...
            if (ob.Id == 0)
            {
                ob.CodeNumber = long.Parse("0" + await GetValueApi<SafeDto>($"GetMax?TypeId={ob.TypeId}")) + 1;
        ...
        public async Task<JsonResult> GetList(string txtSearch = "", int page = 1, int pageSize = 20)
        {
            ...
            var itemsList = await GetListApi<SafeDto>(TextSearch: txtSearch, Page: page, PageSize: pageSize);
