...
namespace OrgSys.Areas.Purchases.Controllers
{
    ...
    public class InvoiceController : BaseController<InvoiceModelView>
    {
        ...
        public override InvoiceModelView InitializeData(InvoiceModelView ob)
        {
            ...
            var StoreId = long.Parse("0" + setting.GetByKey("DefaultStore", "Invoice", 2, 0)?.Value);
            var SupplierId = long.Parse("0" + setting.GetByKey("DefaultSupplier", "Invoice", 2, 0)?.Value);
            var PaymentTypeId = long.Parse("0" + setting.GetByKey("DefaultPaymentType", "Invoice", 2, 0)?.Value);
            var DefaultDiscountType = int.Parse("0" + setting.GetByKey("DefaultDiscountType", "Invoice", 2, 0)?.Value);
            var DefaultServiceType = int.Parse("0" + setting.GetByKey("DefaultServiceType", "Invoice", 2, 0)?.Value);
            var DefaultTaxType = int.Parse("0" + setting.GetByKey("DefaultTaxType", "Invoice", 2, 0)?.Value);
            var DiscountValue = decimal.Parse("0" + setting.GetByKey("DiscountValue", "Invoice", 2, 0)?.Value);
            var ServiceValue = decimal.Parse("0" + setting.GetByKey("ServiceValue", "Invoice", 2, 0)?.Value);
            var TaxValue = decimal.Parse("0" + setting.GetByKey("TaxValue", "Invoice", 2, 0)?.Value);
            ViewBag.NumberLine = int.Parse("0" + setting.GetByKey("NumberLine", "Invoice", 2, 0)?.Value);
            ViewBag.OrderTabe = int.Parse("0" + setting.GetByKey("OrderTabe", "Invoice", 2, 0)?.Value);
            ViewBag.AutoSave = int.Parse("0" + setting.GetByKey("AutoSave", "Invoice", 2, 0)?.Value);
            var TypeCode = int.Parse("0" + setting.GetByKey("TypeSerial", "Invoice", 2, 0)?.Value);
            ViewBag.TypeSerial = TypeCode;
            ViewBag.AllowRepeated = int.Parse("0" + setting.GetByKey("AllowRepeated", "Invoice", 2, 0)?.Value);
            ...
            if (ob.Id == 0)
            {
                ob.CodeNumber = new InvoiceService().GetMaxCode(2);
                ob.Code = "" + new InvoiceService().GetMaxCode(2);
                ob.StoreId = StoreId;
