...
namespace OrgSys.Areas.Financial.Controllers
{
    ...
    public class FinancialController(IConfiguration configuration, IMapper mapper) :
        MainController<FinancialModelView, CreateFinancialCommand , UpdateFinancialCommand>(configuration , mapper)
    {
        public override async Task LoadViewBagIndex(long ParentId = 0, long TypeId = 0)
        {
            ...
            var type = await GetObApi<FinancialTypeModelView>($"GetById?Id={TypeId}");
            ViewBag.FinancialsType = type.Name;
            ViewBag.FinancialsIcon = type.Icon;
        }

        public override Task<FinancialModelView> FixData(FinancialModelView ob)
        {
            if (ob.Id == 0)
            {
        ...
        public override async Task LoadViewBag(FinancialModelView model)
        {
            ...
            ViewBag.OutlayId = new SelectList( await GetListApi<OutlayModelView>($"GetList?ParentId={model.ParentId}"), "Id", "Name", model.OutlayId);
            ...
            ViewBag.SafeId = new SelectList( await GetListApi<SafeModelView>($"GetList?ParentId={model.ParentId}") , "Id", "Name", model.SafeId);
        ...
        public override async Task<FinancialModelView> InitializeData(FinancialModelView ob)
        {


            //var setting = new PreferenceService(User.GetSchema());
            var setting = await GetListApi<PreferenceModelView>(TypeId: ob.TypeId, TextSearch: "Financial", PageSize: 1000);
            ...
            if (ob.Id == 0)
            {
                ...
                ob.CodeNumber = long.Parse("0" + await GetValueApi<FinancialModelView>($"GetMax?ParentId=0&TypeId={ob.TypeId}")) + 1;
                ...
                ob.Rate = (await GetObApi<CurrencyModelView>($"GetById?Id={CurrencyId}")).Rate;
            ...
            ob.SafeName = (await GetObApi<SafeModelView>($"GetById?Id={ob.SafeId}"))?.Name;
        ...
        [HttpPost]
        public async Task<ActionResult> AutoSave(FinancialModelView ob)
        {
            await base.Save(ob);
            if (ob.Id == 0)
            {
                ...
                if (searchResp != null && searchResp.IsSuccessStatusCode)
                {
                    ...
                    var searchRes = JsonConvert.DeserializeObject<ResultPagination<FinancialModelView>>(searchData);
