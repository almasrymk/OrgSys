namespace SaaS.Application.Tenants.Commands
{
    ...
    public sealed record SuspendTenantCommand(long Id) : ICommand, IUpdateCommand<Result>;

    public sealed class SuspendTenantCommandHandler(IUnitOfWork unitOfWork, IRepository<Tenant> repository) : ICommandHandler<SuspendTenantCommand>
    {
        public async Task<Result> Handle(SuspendTenantCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                tenant.Suspend(DateTime.UtcNow);
            }
    ...
    public sealed record CancelTenantCommand(long Id) : ICommand, IUpdateCommand<Result>;

    public sealed class CancelTenantCommandHandler(IUnitOfWork unitOfWork, IRepository<Tenant> repository) : ICommandHandler<CancelTenantCommand>
    {
