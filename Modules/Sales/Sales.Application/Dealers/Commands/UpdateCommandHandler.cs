namespace Sales.Application.Dealers.Commands
{
    ...
    public sealed class UpdateCommandHandler(
        ...
        IRepository<Sales.Domain.Dealer> _Repository,
        IRepository<Account> _AccountRepository,
        IRepository<Preference> _PreferenceRepository,
        IReceivableAccountValidator _Validator,
        IMapper mapper,
        IServiceProvider _provider) : UpdateCommandHandler<UpdateDealerCommand, Sales.Domain.Dealer>(_UnitOfWork, _Repository , mapper , _provider)
    {
        public override async Task<Result> Handle(UpdateDealerCommand request, CancellationToken cancellationToken)
        {
            var dealer = mapper.Map<Sales.Domain.Dealer>(request);

            var (existingAccountId, accountToCreate, errors) = dealer.TypeId == (long)Sales.Domain.DealerType.Supplier
