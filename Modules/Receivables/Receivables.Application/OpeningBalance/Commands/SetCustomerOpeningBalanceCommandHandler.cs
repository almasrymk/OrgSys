namespace Receivables.Application.OpeningBalance.Commands
{
    ...
    /// customer's line inside the single per-fiscal-year Opening Balance Journal that the existing
    /// manual Journal workflow already supports (<c>JournalType.IsOpeningBlance</c>, enforced by
    /// <see cref="IAccountingPeriodService.ValidateOpeningBalanceAsync"/>) — not a parallel per-customer
        ...
        IAccountingPeriodService _AccountingPeriodService) : ICommandHandler<SetCustomerOpeningBalanceCommand>
    {
        public async Task<Result> Handle(SetCustomerOpeningBalanceCommand request, CancellationToken cancellationToken)
        {
            ...
            var (clearingAccount, clearingErrors) = await _Validator.ValidateAccountAsync(clearingAccountId, cancellationToken);
            ...
            try
            {
                ...
                if (journal is null)
                {
                    ...
                    journal = new Journal
                    {
                        ...
                        Note = "Customer Opening Balances"
                    };
