...
public class PaymentApplicationTests
{
    ...
    private static PaymentApplication NewApplication(decimal amount = 1000) =>
        PaymentApplication.Create(sourceFinancialId: 500, customerId: 1, amount: amount, createUserId: 1, createDate: PaymentDate);
    ...
    public void Create_payment_application_with_valid_data()
    {
        ...
        Assert.Equal(1, application.CustomerId);
    ...
    public void RecordLine_reduces_unapplied_amount()
    {
        ...
        application.RecordLine(receivableId: 10, appliedAmount: 400);

        Assert.Equal(600, application.UnappliedAmount);
        var line = Assert.Single(application.Lines);
        Assert.Equal(10, line.ReceivableId);
        Assert.Equal(400, line.Amount);
    }

    [Fact]
    public void RecordLine_can_split_across_multiple_receivables()
    {
        var application = NewApplication(1000);
