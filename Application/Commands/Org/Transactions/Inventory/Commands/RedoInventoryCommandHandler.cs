...
namespace Application.Commands.Org.Transactions.Inventory.Commands
{
    ...
    public sealed class RedoInventoryCommandHandler(
        ...
        : UpdateCommandHandler<RedoInventoryCommand, Domain.Entities.Inventory>(unitOfWork, repository, mapper, provider)
    {
        public override async Task<Result> Handle(RedoInventoryCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (inventory.Status == Domain.Enums.Status.New)
                {
                    await unitOfWork.RollbackAsync();
                    return new Result(HttpStatusCode.OK, null);
                }

                inventory.Status = Domain.Enums.Status.New;
                await new InventoryAdjustmentIntegration(provider)
                    .SetStatusAsync(inventory.Id, Domain.Enums.Status.New);
